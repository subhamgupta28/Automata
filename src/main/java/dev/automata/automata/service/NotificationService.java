package dev.automata.automata.service;

import dev.automata.automata.dto.NotificationAction;
import dev.automata.automata.model.Notification;
import dev.automata.automata.model.Users;
import dev.automata.automata.repository.NotificationRepository;
import jakarta.annotation.PreDestroy;
import lombok.RequiredArgsConstructor;
import lombok.extern.slf4j.Slf4j;
import org.springframework.beans.factory.annotation.Value;
import org.springframework.context.ApplicationEventPublisher;
import org.springframework.data.domain.PageRequest;
import org.springframework.http.HttpEntity;
import org.springframework.http.HttpHeaders;
import org.springframework.http.MediaType;
import org.springframework.messaging.simp.SimpMessagingTemplate;
import org.springframework.stereotype.Service;
import org.springframework.web.client.RestTemplate;

import java.util.Date;
import java.util.List;
import java.util.Map;

@Slf4j
@Service
@RequiredArgsConstructor
public class NotificationService {
    private final SimpMessagingTemplate messagingTemplate;
    private final NotificationRepository notificationRepository;
    private final ApplicationEventPublisher publisher;
    private RestTemplate restTemplate;
    private static final String SUCCESS = "success";
    private static final String TITLE_AUTOMATA = "Automata";

    @Value("${app.notification.ntfy.url}")
    private String NTFY_URL;

    public void sendAlert(String message, String severity) {
        Notification notification = Notification.builder()
                .message(message)
                .severity(severity)
                .timestamp(new Date())
                .build();
        messagingTemplate.convertAndSend("/topic/alert", notification);
    }

    public void sendNotification(String message, String severity, String header, String homeId, String... automationIds) {
        Notification notification = Notification.builder()
                .message(message)
                .severity(severity)
                .automationId(automationIds.length != 0 ? automationIds[0] : "")
                .header(header)
                .timestamp(new Date())
                .build();
        log.info("Notification sent: message={} severity={} homeId={}", message, severity, homeId);
        if ("high".equals(severity))
            sendNotify(TITLE_AUTOMATA, message, severity);
        messagingTemplate.convertAndSend("/topic/" + homeId + "/notification", notification);
    }


    public String notificationAction(String action, Map<String, Object> payload, String homeId, Users user) {
        var event = NotificationAction.builder()
                .action(action)
                .payload(payload)
                .user(user)
                .homeId(homeId)
                .build();
        publisher.publishEvent(event);
        return SUCCESS;
    }

    public List<Notification> getLastFiveNotifications() {
        return notificationRepository.findAllBySeverityIsOrderByTimestampDesc("medium", PageRequest.of(0, 5));
    }


    public String test(String type) {
        sendAlert("Test", type);
        return SUCCESS;
    }

    public void sendNotify(String title, String message, String priority) {
        try {
            restTemplate = new RestTemplate();
            HttpHeaders headers = new HttpHeaders();
            headers.setContentType(MediaType.TEXT_PLAIN);
            headers.add("Title", title);
            headers.add("Priority", priority);
//        headers.add("Tags", "warning");

            log.info("Sending notification to url {} with message {}", NTFY_URL, message);
            HttpEntity<String> request = new HttpEntity<>(message, headers);
            restTemplate.postForObject(NTFY_URL, request, String.class);
        } catch (Exception e) {
            log.error("ERROR: sendNotify", e);
        }
    }

    @PreDestroy
    public void onShutdown() {
        try {
            sendNotify(TITLE_AUTOMATA, "Server Shutting Down", "urgent");
            log.info("Shutdown API called successfully.");
        } catch (Exception e) {
            log.error("Failed to call shutdown API:", e);
        }
    }

    @Deprecated(since = "29th July, 2026", forRemoval = true)
    public void sendNotification(String message, String severity, String homeId) {
        Notification notification = Notification.builder()
                .message(message)
                .severity(severity)
                .timestamp(new Date())
                .build();

        log.info("Notification sent: message= {} home id= {}", message, homeId);
        if (severity.equals("high"))
            sendNotify(TITLE_AUTOMATA, message, severity);
        messagingTemplate.convertAndSend("/topic/" + homeId + "/notification", notification);
    }
}
