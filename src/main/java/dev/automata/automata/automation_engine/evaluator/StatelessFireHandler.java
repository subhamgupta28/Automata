...
@Component
@Slf4j
public class StatelessFireHandler implements OutcomeHandler {
    @Override
    public EvalOutcome outcome() {
        return EvalOutcome.STATELESS_FIRE;
    }
    ...
    public void applyStateTransition(AutomationRuntimeState next, EvalResult result, ExecutionPlan plan) {
        /* no-op — not handled by the old switch either */
        log.debug("[{}] StatelessFireHandler: next={} result={}", plan.getAutomationName(), next.getTopLevelState(), result.getOutcome());
    }
